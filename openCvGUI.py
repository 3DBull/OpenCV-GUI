...
class Button():
    ...
    def sizeTo(self, prop: str):
        if prop == 'text':
            ...
            self.text.fsize, self.text.thickness, self.text.size = Text.autosizeText(self.text.text,
                                                                                     addT(self.image.shape[:2],
                                                                                          mult(self.pad, -2)))
            textBound, base = cv2.getTextSize(self.text.text, Theme.font, self.text.fsize, self.text.thickness)
    ...
    def draw(self, window):
        if self.image.shape[0] == 0:
            if self.highlight:
                cv2.rectangle(window, flip(self.pos), flip(addT(self.pos, self.size)), Theme.highlightColor,
                ...
                cv2.rectangle(window, flip(self.pos), flip(addT(self.pos, self.size)), Theme.backgroundColor,
                              thickness=cv2.FILLED)
                cv2.rectangle(window, flip(self.pos), flip(addT(self.pos, self.size)), Theme.mainColor, thickness=2)
...
class Text():
    ...
    def draw(self, window, color=None):
        if color == None:
            color = self.color
        cv2.putText(window, self.text, (self.pos[1], self.pos[0] + self.size[0]), Theme.font, self.fsize, color,
                    self.thickness)
...
class Container:
    ...
    def autoArrange(self, style='GRID'):
        ...
        if style == 'GRID':
            ...
            for n in self.items:
                if n not in self.hidden:
                    self.items[n].pos = (place + space,
                                         self.items[n].pos[1] if style == 'VERTICAL' else self.size[1] // 2 -
                                                                                          self.items[n].size[1] // 2)
            ...
            for n in self.items:
                if n not in self.hidden:
                    self.items[n].pos = (
                    self.items[n].pos[0] if style == 'HORIZONTAL' else self.size[0] // 2 - self.items[n].size[0] // 2,
                    place + space)
                    if isinstance(self.items[n], Button):
                        self.items[n].text.pos = addT(self.items[n].pos, self.items[n].pad)
...
class Window:
    ...
    def show(self, pos=None):
        ...
        if self.size == screenSize:
            ...
            # add boarder
            cv2.rectangle(screen, addT(flip(self.pos), (-3, -3)), flip(addT(self.pos, self.size, (3, 3))),
                          addLT(Theme.highlightColor, (-30, -30, -30)), 4)
    ...
    def autoArrange(self, style='VERTICAL_CENTERED'):
        ...
        if style == 'GRID':
            ...
            place = 0
            for n in self.items:
                self.items[n].pos = (place + space, self.items[n].pos[1] if style == 'VERTICAL' else self.size[1] // 2 -
                                                                                                     self.items[n].size[
                                                                                                         1] // 2)
                if isinstance(self.items[n], Button):
                    self.items[n].text.pos = addT(self.items[n].pos, self.items[n].pad)
